...
<html>
<head>
    <title>E-Legend | eJackpot | Login Callback</title>
    <style>
        body {
            position: relative;
            margin: 0;
...
<script>
    ...
    client.readSigninResponseState(window.location.href, null, true).then(async signState => {
        console.log('---------> signState', signState)
        try {
            var response = await axios.post('{{ route('client.user.post-login') }}', {
                code: signState.response.code,
                code_challenge: signState.state._code_challenge,
                code_verifier: signState.state._code_verifier
            });
            console.log(response.data);
            window.location.replace(response.data.client_redirect_uri);
        } catch (e) {
            window.location.replace('{{ route('client.home.index') }}');
        }
