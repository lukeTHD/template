        ...
        <div class="kt-portlet kt-portlet--last kt-portlet--head-lg kt-portlet--responsive-mobile" id="kt_page_portlet">
            <div class="kt-portlet__head kt-portlet__head--lg">
                <div class="kt-portlet__head-label">
                    ...
                    <h3 class="kt-portlet__head-title">{{ __('label.metabox.list') }} </h3>
                </div>
                <div class="kt-portlet__head-toolbar">
                    <button type="button" v-on:click="addMetaBox()" class="btn btn-success mr-3">
                        <i class="la la-plus"></i>
                        <span class="kt-hidden-mobile">{{ __('label.metabox.add') }}</span>
                    </button>
            ...
            <div class="kt-portlet__body">
                <form class="kt-form" action="{{ route('admin.metabox.store') }}" method="POST" id="kt_form">
                    @csrf
                    <textarea name="metabox" class="form-control d-none"><% MetaboxJson %></textarea>
                    <div class="row">
                            ...
                            <div class="kt-section kt-section--last">
                                <div class="kt-section__body">
                                    <div class="accordion accordion-solid accordion-panel accordion-toggle-svg"
                                        ...
                                        <div class="card" v-for="(metabox,key) in metaboxes">
                                            <div class="card-header" id="headingOne8">
                                                <div class="card-title border" data-toggle="collapse"
                                                     :data-target="bindDataTarget(key)" aria-expanded="true"
                                                     aria-controls="collapseOne8">
                                                    <span v-if="metabox.meta_key"><% metabox.meta_key %></span>
                                                    <span v-else><i>Chưa đặt key</i></span>
                                            ...
                                            <div v-bind:id="bindDataTarget(key,true)" class="collapse"
                                                ...
                                                <div class="card-body border border-top-0 p-3">
                                                    <div class="form-group">
                                                        <label>Key</label>
                                                        <input v-model="metabox.meta_key" type="text"
                                                               class="form-control rounded-0">
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Value</label>
                                                        <textarea rows="3" v-model="metabox.meta_value"
                                                                  class="form-control rounded-0"></textarea>
                                                    </div>
                                                    <div class="form-group form-group-last text-right">
                                                        <button type="button" aria-expanded="false" v-on:click="removeMetaBox($event,key)"
    ...
    <script>
        new Vue({
            ...
            methods: {
                ...
                removeMetaBox: function (event, key) {
                    event.preventDefault();
                    this.metaboxes = this.metaboxes.filter(function (item, index) {
                        return index !== key;
                    });
                }
            },
            computed: {
                MetaboxJson: function () {
                    return JSON.stringify(this.metaboxes);
                }
            }
        })
    </script>
