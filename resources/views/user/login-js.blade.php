			...
			KTUtil.getById('kt_login_signin_form'),
			{
				fields: {
					username: {
						validators: {
							notEmpty: {
								message: "{{__('label.login.username_required')}}"
                            },
                            emailAddress: {
                                message: "{{__('label.login.username_is_email')}}"
                            }
						}
					},
					password: {
						validators: {
							notEmpty: {
								message: "{{__('label.login.password_required')}}"
							}
						}
					}
				},
        ...
        $('#kt_login_signin_submit').on('click', function (e) {
            ...
            validation.validate().then(function(status) {
		        if (status == 'Valid') {
					...
					swal.fire({
		                text: "{{__('label.login.error_login')}}",
		                icon: "error",
